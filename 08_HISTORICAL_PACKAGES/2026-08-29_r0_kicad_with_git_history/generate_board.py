...
FP_ROOT = Path(os.environ.get(
    "KICAD_FOOTPRINTS",
    "/usr/share/kicad/footprints" if Path("/usr/share/kicad/footprints").exists()
    else "/Applications/KiCad.app/Contents/SharedSupport/footprints",
))
...
def add_smd_pad(fp: pcbnew.FOOTPRINT, number: str, x: float, y: float,
    ...
    pad.SetFPRelativePosition(vec(x, y))
...
def fp_mic(ref: str, x: float, y: float, rot: float = 0.0) -> pcbnew.FOOTPRINT:
    ...
    # Spread >=0.15 mm from the 0.60 mm hole for the board hole-clearance rule.
    for dx, dy in [(0, -0.97), (+0.80, -0.36), (0, +0.28), (-0.80, -0.36)]:
...
U2 = fp_npm1300("U2", 32.7, 30.0)
U3 = load_fp("Package_LGA", "LGA-12_2x2mm_P0.5mm", "U3", "LIS2DW12TR", 48.0, 36.1)
U4 = fp_tpd2("U4", 28.8, 30.0, 90)
U4.Flip(U4.GetPosition(), False)
MIC1 = fp_mic("MIC1", 38.0, 22.3, 0)
MIC2 = fp_mic("MIC2", 51.0, 22.3, 0)
# USB4105 origin is the connector body centre: mating face at local y=-3.675,
# solder pads at local y=-3.68 (the back edge). Rotated 270 deg the face sits
# at x = x_center - 3.675; place the face flush with the left board edge (20).
J1 = load_fp("Connector_USB", "USB_C_Receptacle_GCT_USB4105-xx-A_16P_TopMnt_Horizontal",
             "J1", "USB4105-GF-A", 23.675, 30.0, 270)
...
LED1 = fp_led("LED1", 27.0, 38.0)
Q1 = load_fp("Package_TO_SOT_SMD", "SOT-23", "Q1", "AO3400A", 37.0, 37.0, 180)
D1 = load_fp("Diode_SMD", "D_SOD-323", "D1", "1N4148W-HF", 41.0, 37.0)
BT1 = fp_solder_3("BT1", "LiPo_1S_PROTECTED_NTC", 44.6, 37.6)
M1 = fp_solder_2("M1", "VCLP1020B002L", 34.5, 37.0)

L1 = load_fp("Inductor_SMD", "L_Murata_DFE201610P", "L1", "2.2uH CIGT201610EH2R2MNE", 37.8, 27.2)
L2 = load_fp("Inductor_SMD", "L_Murata_DFE201610P", "L2", "2.2uH CIGT201610EH2R2MNE", 37.8, 32.8)
L3 = load_fp("Inductor_SMD", "L_Murata_DFE201610P", "L3", "10uH MLZ2012N100LT000", 51.1, 33.2)
...
C1 = add_c("C1", "1uF 16V X7R", 37.6, 29.1, "0603")
C2 = add_c("C2", "10uF 10V X5R", 27.8, 23.2, "0603")
C3 = add_c("C3", "10uF 10V X5R", 28.7, 28.0, "0603", 90, bottom=True)
C4 = add_c("C4", "10uF 10V X5R", 26.9, 36.6, "0603")
C5 = add_c("C5", "1uF 10V X7R", 37.6, 30.9, "0603")
C6 = add_c("C6", "2.2uF 16V X7R EMK107BB7225KA-T", 32.7, 30.0, "0603", bottom=True)
C7 = add_c("C7", "10uF 10V X5R", 40.4, 27.2, "0603", 90)
C8 = add_c("C8", "10uF 10V X5R", 40.4, 32.8, "0603", 90)
C9 = add_c("C9", "10uF 10V X5R", 42.0, 29.9, "0603")
...
C17 = add_c("C17", "1uF MIC BYPASS", 40.6, 21.0, "0603")
C18 = add_c("C18", "1uF MIC BYPASS", 54.1, 22.2, "0603")
C19 = add_c("C19", "10uF MCU BULK", 51.0, 35.9, "0603")
C20 = add_c("C20", "100nF ACC BYPASS", 50.7, 37.2)
C21 = add_c("C21", "47uF SD BULK", 38.6, 38.0, "0805", bottom=True)
C22 = add_c("C22", "100nF SD BYPASS", 35.4, 38.0, bottom=True)
C23 = add_c("C23", "100nF MOTOR", 40.6, 35.4)
C24 = add_c("C24", "47uF MOTOR BULK", 32.2, 37.9, "0805", 90, bottom=True)

R1 = add_r("R1", "10k I2C", 36.4, 25.4)
R2 = add_r("R2", "10k I2C", 30.8, 34.4)
R3 = add_r("R3", "330k 1% BUCK1=2.7V START", 28.8, 35.0, bottom=True)
R4 = add_r("R4", "150k 1% BUCK2=3.0V", 30.6, 33.8, bottom=True)
R5 = add_r("R5", "5.1k USB CC1", 23.9, 24.0, bottom=True)
R6 = add_r("R6", "5.1k USB CC2", 26.4, 24.0, bottom=True)
R7 = add_r("R7", "100k SD CS PULLUP", 41.6, 38.0, bottom=True)
R8 = add_r("R8", "22R SD SCK", 43.8, 38.0, bottom=True)
R9 = add_r("R9", "22R SD MOSI", 46.0, 38.0, bottom=True)
R10 = add_r("R10", "22R SD MISO", 48.2, 38.0, bottom=True)
R11 = add_r("R11", "100k BUTTON PULLUP", 32.0, 35.4, bottom=True)
R12 = add_r("R12", "100R HAPTIC GATE", 31.5, 39.2)
R13 = add_r("R13", "100k HAPTIC PULLDOWN", 33.6, 39.2)
R14 = add_r("R14", "100R MIC1 DATA", 41.2, 23.3)
R15 = add_r("R15", "100R MIC2 DATA", 56.9, 23.5)
...
for i, (ref, name) in enumerate(TEST_NETS):
    tp = fp_testpad(ref, name, 50.5 + i * 1.5, 38.3)
...
set_many(U1, {
    ...
    "28": "3V_MAIN", "30": "3V_MAIN", "31": "DCCH", "32": "VBUS",
...
set_many(J2, {"2": "SD_CS", "3": "SD_MOSI_CARD", "4": "3V_SD", "5": "SD_SCK_CARD",
              "6": "GND", "7": "SD_MISO_CARD", "9": "SD_DETECT", "10": "GND", "SH": "GND"})
...
for label, x, y, layer in [
    ("ANT-PROD-R0 EVT", 40.0, 39.3, pcbnew.F_SilkS),
]:
    add_text(label, x, y, layer, 0.8)
add_text("ANTENNA - NO COPPER / BATTERY / METAL", 66.8, 30.0, pcbnew.Dwgs_User, 0.8)
...
# Hole-to-copper clearance: 0.15 mm. The GCT USB4105 shell stakes (same-net
# NPTH vs the shell SMT lands, official KiCad footprint geometry) sit at
# 0.19 mm; same-net shell lands merge in fab. All other holes clear 0.25 mm
# in practice; assembler DFM arbitrates before ordering. Documented in
# docs/ENGINEERING_AUDIT.md and RELEASE_GATES.md.
board.GetDesignSettings().m_HoleClearance = mm(0.25)
# 0.4 mm copper-to-edge: above the assembler 0.3 mm minimum, documented.
board.GetDesignSettings().m_CopperEdgeClearance = mm(0.4)
