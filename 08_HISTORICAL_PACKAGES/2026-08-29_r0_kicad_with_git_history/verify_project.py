...
require(len(footprints) == 63, f"expected 63 footprints, found {len(footprints)}")
...
for ref, pin, expected in [
    ...
    ("U1", "31", "DCCH"), ("L3", "1", "DCCH"), ("L3", "2", "3V_MAIN"),
]:
...
subprocess.run(
    ["/Applications/KiCad.app/Contents/MacOS/kicad-cli", "pcb", "drc",
     "--output", str(REPORT_PATH), "--format", "report", "--severity-all",
     str(BOARD_PATH)],
    check=False, capture_output=True,
)
drc = REPORT_PATH.read_text(errors="replace")
error_count = drc.count("; error")
