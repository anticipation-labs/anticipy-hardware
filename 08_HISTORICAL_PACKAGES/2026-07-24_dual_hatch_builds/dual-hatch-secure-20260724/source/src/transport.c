...
/*
 * CLOSED. The security debt recorded by the bridge image is paid here.
 *
 * The bridge image used BT_GATT_PERM_WRITE (0x02: no encryption, no bonding)
 * on this control point, because an open door we could reach beat a locked
 * door we could not -- this pendant has no usable RESET button, and at that
 * point the wireless hatch was the only route into the bootloader that had
 * ever been demonstrated. The cost was that ANY peer in radio range could
 * write 0x06 and force a bootloader entry (denial of service only: no audio
 * access, no data exfiltration).
 *
 * Both preconditions named in that debt note are now satisfied:
 *
 *   1. A second, independent recovery route has been PHYSICALLY CONFIRMED on
 *      the hardware -- the 1200-baud USB CDC touch in recovery_usb.c drives
 *      GPREGRET 0x57 and mounts the UF2 volume with no hands and no radio.
 *      That is now the PRIMARY recovery route; the BLE hatch is the backup.
 *   2. Bonding is actually reachable: CONFIG_BT_SMP, LE Secure Connections
 *      only, CONFIG_BT_BONDING_REQUIRED and persistent bonds via
 *      CONFIG_BT_SETTINGS/NVS are all enabled in the project configuration,
 *      and the audio path already depends on them.
 *
 * So the control point now takes BT_GATT_PERM_WRITE_ENCRYPT (0x08): only a
 * bonded peer on an encrypted link can request a bootloader entry. An
 * unbonded peer's write is rejected by the GATT layer with
 * BT_ATT_ERR_INSUFFICIENT_ENCRYPTION before this file's handler is ever
 * reached.
 *
 * A bricking analysis, since this closes the door we used to rely on:
 *   - Bond lost / phone forgot the device: CONFIG_BT_KEYS_OVERWRITE_OLDEST=y
 *     lets a fresh pairing evict the stale bond, so re-pairing still works.
 *     Without it, CONFIG_BT_MAX_PAIRED=1 would make a lost bond a permanent
 *     wireless lockout.
 *   - Bonding itself broken, or no phone available: the cable hatch does not
 *     use the radio, the GATT layer, or any bond, and runs before everything
 *     else in main(). It is unaffected by anything decided here.
 *
 * The CCC below is deliberately left unencrypted. It only controls delivery
 * of the 0x10 acknowledgement notification; subscribing to it cannot reset
 * the device, and the value write that can is now encryption-gated.
 *
 * REMAINING GAP (not a regression, and not closable in firmware alone):
 * there is still no physical enrollment or bond-erase gesture, so the first
 * central to pair becomes the sole stored bond, and with OVERWRITE_OLDEST a
 * peer that can complete a Just Works pairing can displace it. This narrows
 * the DFU attack surface from "any radio in range" to "a peer that can
 * complete an LE Secure Connections pairing"; it does not eliminate it.
 */
...
int transport_start(void)
{
    ...
    if (dfu_error != 0) {
        ...
        LOG_INF("Legacy DFU service registered "
                "(control point requires an encrypted, bonded link)");
    }
