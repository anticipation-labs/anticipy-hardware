...
private final class OmiBLEProbe: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  func centralManagerDidUpdateState(_ central: CBCentralManager) {
    ...
    // Scan UNFILTERED and match ourselves. A service-filtered scan is unreliable on macOS here:
    // it has returned unrelated system-connected devices while missing the pendant entirely, even
    // when an unfiltered sweep saw the pendant advertising 19B10000 clearly.
    print("EVENT scan service=UNFILTERED_MATCH_IN_CODE")
  ...
  func centralManager(
    ...
    let isPendant = advSvcs.contains(audioServiceUUID) || advName == "Anticipy" || advName == "Friend"
    guard isPendant else { return }
    print("EVENT matched name=\"\(advName)\" rssi=\(RSSI)")
...
private final class ProbeAppDelegate: NSObject, NSApplicationDelegate {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    // AUTO-START: this run is unattended. NSApplication is retained only so macOS can present the
    // Bluetooth permission prompt; the probe begins immediately rather than waiting on a click.
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) { [weak self] in self?.startProbe() }
  }
...
private let reconLogPath = ("~/anticipy-pendant-restore/tools/recon/AUDIO_CAPTURE.txt" as NSString)
