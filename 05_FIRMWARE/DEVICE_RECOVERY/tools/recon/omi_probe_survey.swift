...
private final class OmiBLEProbe: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  func centralManagerDidUpdateState(_ central: CBCentralManager) {
    ...
    // SURVEY MODE: scan UNFILTERED. A service-filtered scan returned a system-connected Apple
    // device with advertisedServices=[], so we enumerate everything and log each advertisement
    // instead of trusting the filter. Still read-only: discovery does not connect or write.
    print("EVENT scan service=ALL_UNFILTERED")
  ...
  func centralManager(
    ...
    let advSvcs = ((advertisementData[CBAdvertisementDataServiceUUIDsKey] as? [CBUUID]) ?? [])
      .map { $0.uuidString }
    print("SURVEY name=\"\(advName)\" rssi=\(RSSI) services=\(advSvcs) id=\(peripheral.identifier)")
    return  // SURVEY MODE: enumerate only. Do not connect, do not stop the scan.
...
private let reconLogPath = ("~/anticipy-pendant-restore/tools/recon/SURVEY_RESULT.txt" as NSString)
