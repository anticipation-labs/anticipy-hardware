...
/*
 * DIAGNOSTIC BUILD ONLY (dual-hatch-debug-20260724).
 *
 * Startup return values, captured where they cannot be optimised away.
 *
 * In the shipping image CONFIG_LOG is off, so every LOG_ERR/LOG_INF below
 * compiles to nothing and the DFU registration result is genuinely discarded
 * by the compiler -- disassembly of the shipping binary at 0x3949E shows
 * "bl bt_gatt_service_register" followed immediately by "mov r0, r4", which
 * reloads the AUDIO service's result. A registration failure there is
 * therefore invisible in the running firmware.
 *
 * These are `volatile` so the stores survive regardless of what the logging
 * configuration does, and they are readable from a debugger or a crash dump
 * even if nothing ever reaches the console. TRANSPORT_START_NOT_REACHED is a
 * deliberately impossible errno so "never ran" is distinguishable from
 * "ran and returned 0".
 */
#define TRANSPORT_START_NOT_REACHED 0x7FFFFFFF

volatile int transport_audio_service_register_result =
    TRANSPORT_START_NOT_REACHED;
volatile int transport_dfu_service_register_result =
    TRANSPORT_START_NOT_REACHED;
volatile int transport_bt_enable_result =
    TRANSPORT_START_NOT_REACHED;
volatile int transport_settings_load_result =
    TRANSPORT_START_NOT_REACHED;
volatile int transport_adv_start_result =
    TRANSPORT_START_NOT_REACHED;

/*
 * printk, not LOG_*, on purpose: CONFIG_LOG_PRINTK=n in this image, so these
 * lines go straight out through the console hook and cannot be dropped by a
 * full deferred log buffer or reordered behind the Bluetooth debug torrent.
 */
#define TRANSPORT_DIAG(fmt, ...) \
    printk("[ANTICIPY-DIAG] " fmt "\n", ##__VA_ARGS__)
...
int transport_start(void)
{
     ...
     */
    TRANSPORT_DIAG(
        "transport_start: registering audio service (%u attrs) pre-bt_enable",
        (unsigned int)ARRAY_SIZE(audio_service_attr));
    int err = bt_gatt_service_register(&audio_service);
    transport_audio_service_register_result = err;
    TRANSPORT_DIAG(
        "bt_gatt_service_register(audio_service) = %d", err);
    if (err != 0) {
     ...
     */
    TRANSPORT_DIAG(
        "transport_start: registering legacy DFU service (%u attrs) "
        "pre-bt_enable",
        (unsigned int)ARRAY_SIZE(dfu_service_attr));
    int dfu_error = bt_gatt_service_register(&dfu_service);
    /*
     * PRIME-SUSPECT INSTRUMENTATION. This return value is discarded in the
     * shipping binary. Store it and print it before anything else can clobber
     * r0, so a non-zero result here is impossible to miss.
     */
    transport_dfu_service_register_result = dfu_error;
    TRANSPORT_DIAG(
        "bt_gatt_service_register(dfu_service) = %d", dfu_error);
    if (dfu_error != 0) {
        TRANSPORT_DIAG(
            "*** DFU SERVICE REGISTRATION FAILED (%d) *** the GATT attribute "
            "table may be partially populated; wireless recovery is "
            "unavailable, cable recovery (GPREGRET 0x57) is unaffected",
            dfu_error);
    ...
    err = bt_enable(NULL);
    transport_bt_enable_result = err;
    TRANSPORT_DIAG("bt_enable(NULL) = %d", err);
    if (err != 0) {
    ...
    err = settings_load();
    transport_settings_load_result = err;
    TRANSPORT_DIAG("settings_load() = %d", err);
    if (err != 0) {
    ...
    err = bt_le_adv_start(
        ...
        scan_response_data,
        ARRAY_SIZE(scan_response_data));
    transport_adv_start_result = err;
    TRANSPORT_DIAG("bt_le_adv_start(BT_LE_ADV_CONN) = %d", err);
    if (err != 0) {
        /*
