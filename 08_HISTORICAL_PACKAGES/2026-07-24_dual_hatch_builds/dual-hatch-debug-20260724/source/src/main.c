...
int main(void)
{
     ...
     */
    /*
     * DIAGNOSTIC BUILD banner. Emitted here rather than at the top of main()
     * on purpose: CONFIG_USB_DEVICE_INITIALIZE_AT_BOOT=n, so usb_enable() only
     * runs inside recovery_usb_start() above, and cdc_acm_poll_out() silently
     * drops every character until the host has finished enumerating. By this
     * point boot_led_sequence() has burned ~3 s, so the endpoint is up and this
     * line actually reaches the console. Anything printed before it is
     * expected to be missing.
     */
    printk("\n[ANTICIPY-DIAG] dual-hatch-debug-20260724 boot: "
           "codec+mic ready, starting transport\n");
