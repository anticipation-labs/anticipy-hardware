...
static struct bt_gatt_attr button_service_attr[] = {
    ...
    BT_GATT_CHARACTERISTIC(&button_characteristic_data_uuid.uuid,
                           ...
                           NULL,
                           NULL),
    BT_GATT_CCC(button_ccc_config_changed_handler,
...
void turnoff_all()
{
    ...
    set_led_red(false);
    set_led_green(false);
    gpio_remove_callback(button_pin.port, &button_cb_data);
    gpio_pin_interrupt_configure_dt(&button_pin, GPIO_INT_LEVEL_ACTIVE);
