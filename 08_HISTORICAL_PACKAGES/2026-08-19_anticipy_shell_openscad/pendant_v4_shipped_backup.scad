...
LIP_CLR   = (VARIANT == 4) ? 0.28 : 0.16;   // per side
...
BAY_T   = max(BRD_T, BAT_T) + 0.5 + DEPTH_PAD;   // one shared layer
D       = BAY_T + 2*WALL;
...
H = BAT_BAY_L + GAP_BB + BRD_BAY_L + BAIL_GAP + BAIL_D + BAIL_TOP + 1.2;
...
BAT_ZF     = Z_FRONT_IN - 0.4;               // battery front face
...
module usb_hole() {
    ...
    intersection() {
        ...
        translate([W/2 - 3, USB_Y, USB_Z]) rotate([0,90,0])
            hull() for (sx=[-1,1], sy=[-1,1])
                translate([sx*(7.4/2 - 2), sy*(13.4/2 - 2), 0])
