...
// HEADLESS VARIANT — 2026-07-24.
// Identical probe logic to omi_ble_probe.swift; only the GUI Start-button gate is replaced by a
// direct start, because this run is autonomous. It remains strictly READ-ONLY: the probe reads the
// codec and battery characteristics and subscribes to audio notifications. It contains no
// writeValue call, and in particular never touches the DFU control point 00001531-…, a single
// write to which would reboot the device into the bootloader.
private let secondsArgument = CommandLine.arguments
  .dropFirst()
  .first.flatMap(Double.init) ?? 25
setvbuf(stdout, nil, _IONBF, 0)
print("EVENT headless_start capture_seconds=\(secondsArgument)")
private let probe = OmiBLEProbe(captureSeconds: secondsArgument)
_ = probe
RunLoop.main.run()
