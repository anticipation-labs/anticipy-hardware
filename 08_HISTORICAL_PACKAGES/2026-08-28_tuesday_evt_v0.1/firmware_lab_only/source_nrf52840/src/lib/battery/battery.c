...
{
    /* Seeed specifies LOW to connect the divider to P0.31/AIN7. */
...
int battery_set_fast_charge()
{
    ...
    /* Charging current is a hardware setting on the XIAO, not firmware control. */
    return 0;
}
...
int battery_charge_start()
{
    ...
    /* The onboard charger runs autonomously whenever USB is present. */
...
int battery_charge_stop()
{
    ...
    /* Do not drive P0.17: Seeed documents it as charge-status. */
    return 0;
}
...
int battery_get_millivolt(uint16_t *battery_millivolt)
{
    ...
    *battery_millivolt = (uint16_t) (((uint32_t) adc_mv * (R1 + R2)) / R2);
...
int battery_init()
{
    ...
    ret |= gpio_pin_configure(gpio_battery_dev, GPIO_BATTERY_READ_ENABLE, GPIO_OUTPUT_LOW);
